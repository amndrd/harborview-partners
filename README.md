...
## Flou progressif aux bords de l'écran (accueil)

Ressource reprise de [details.so/vault/blur-01](https://www.details.so/vault/blur-01),
dans sa forme d'origine. Huit couches de `backdrop-filter` par bord, dont la
force double d'une couche à la suivante (0,078125 px → 10 px), chacune découpée
par un masque qui la cantonne à une bande de 12,5 % de la hauteur. Les bandes se
chevauchent, et c'est ce chevauchement qui donne un dégradé de netteté continu.

Fichiers : `assets/progressive-blur.css`, `assets/progressive-blur.js`, plus le
balisage dans `index.html`.

Le point à ne pas rater : cela ne se résume pas à un seul élément flouté sous un
masque. Un masque unique fait varier l'opacité d'un flou de force constante ; il
ne reproduit pas l'augmentation par paliers de la force elle-même. Les valeurs de
flou et les arrêts de masque sont donc repris tels quels — les toucher change la
courbe. Les `#000` des dégradés ne peignent rien : dans un `mask-image`, seule
l'opacité compte, et l'effet n'a donc pas de couleur propre.

### Ce qui a été vérifié avant de l'installer

**`position: fixed` tient.** La ressource prévient qu'un ancêtre transformé
changerait le bloc conteneur et décrocherait l'effet du cadre. Toute la chaîne
au-dessus de `.body-inner` a été inspectée, à 1440 comme à 393 px : aucun
`transform`, `filter`, `perspective`, `will-change` ni `contain` qui en créerait
un. Vérifié aussi en mouvement sous 768 px, là où ce n'est pas la fenêtre qui
défile mais `.body-inner` : le rectangle reste à `[-32, 160]` avant et après
défilement, il ne dérive pas avec le conteneur.

**Le `z-index: 10` d'origine convient sans retouche.** Il tombe exactement où la
ressource le veut — au-dessus du contenu, en dessous des commandes qui doivent
rester nettes. L'échelle de cette page : `.article-layout` 1, `.main` 2,
`.home-hero-stick` 3, `.header` 100, `#cookie-consent` 600. La barre de
navigation et le bandeau cookies passent donc par-dessus sans être floutés,
tandis que le contenu qui défile sous la barre, lui, l'est.

**La sonde de contraste ne le voit pas.** `header-contrast.js` compose les
surfaces réellement peintes sous la rangée. Les couches n'ont aucune couleur de
fond (la ressource l'interdit) et portent `pointer-events: none` : elles ne sont
donc ni candidates ni présentes dans la pile du test de survol. Vérifié par
échantillonnage dense du bord de carte contre l'état de la barre, flou actif : la
bascule tombe toujours entre y = 50 et y = 48, sans aucun écart — les mêmes
valeurs qu'avant l'ajout.

### Le déclenchement après le hero

Seul ajout à la ressource, et il ne touche pas à sa mécanique. L'effet est
demandé une fois le hero passé, pas au-dessus de lui : `progressive-blur.js` pose
une classe quand le hero quitte l'écran, et la retire quand il revient. Un
`IntersectionObserver`, qui ne se réveille qu'aux deux franchissements du seuil,
et qui se moque de savoir qui défile. Rien d'autre n'écoute le défilement : c'est
bien le `position: fixed` et la recomposition du fond qui font l'effet.

Tenir les couches masquées quand elles ne servent pas n'est pas cosmétique.
Seize `backdrop-filter` composés en permanence sur une page qui porte déjà un
canvas animé, un défilement lissé et une pile de cartes épinglée, cela se
paierait ; `visibility: hidden` laisse le navigateur les ignorer.

Les deux conteneurs sont posés en enfants directs de `.body-inner`, à côté de
`.main` — c'est-à-dire hors du conteneur Barba, comme la barre de navigation.
Ils survivent ainsi aux changements de page. Le hero, lui, est dans le conteneur
et disparaît avec lui : l'observateur est remonté à chaque navigation, et l'effet
se désarme si la page d'arrivée n'a pas de hero.

### Vérifié

Structure conforme à la ressource : deux conteneurs, huit couches chacun,
`z-index` de 1 à 8 dans l'ordre, `position: fixed`, `overflow: hidden`,
`pointer-events: none`, décalages de −32 px, déclarations préfixées et non
préfixées conservées. Les valeurs de flou sortent à `0.078125px` pour la première
couche et `10px` pour la huitième. Le palier de la ressource bascule au bon
endroit : 200 px de hauteur à 681 px de large, 160 px à 680.

La bascule se fait exactement au franchissement du bas du hero (mesuré : 99 →
−115), et le retrait au retour. Neuf largeurs de 320 à 1728, aucun débord
latéral, aucune erreur console. Le balisage n'est posé que sur l'accueil, la
seule page dont le hero commande l'effet.
